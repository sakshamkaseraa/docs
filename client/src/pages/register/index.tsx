  ...
  const register = async () => {
    ...
    try {
      ...
      if (axios.isAxiosError(err) && err.response?.data) {
        ...
        if (password1FieldErrors.length) setPassword1Errors(password1FieldErrors);
        if (password2FieldErrors.length) setPassword2Errors(password2FieldErrors);

        if (
          !emailFieldErrors.length &&
          !password1FieldErrors.length &&
          !password2FieldErrors.length
        ) {
